...
Le vecteur $\vec{u}=\vec{AB}$ a donc pour coordonnées $\Coord{4}{1}$.

On peut également le noter $\vec{AB}\Coord{4}{1}$
...
\begin{prop}
    Soient deux points $M(x_M,y_M)$ et $M'(x_{M'},y_{M'})$ dans un repère. Les coordonnées du vecteur $\vec{MM'}$ sont $\Coord{x_{M'}-x_{M}}{y_{M'}-y_{M}}$.
\end{prop}
...
\begin{de}
    Le vecteur nul est le vecteur de coordonnées $\Coord{0}{0}$. Il est le vecteur associé à la transflation identité, qui transforme tout point en lui-même.
\end{de}
...
Effectuer plusieurs translations d'affilée revient à effectuer une seule translation, associée à un vecteur qui correspond à la somme des vecteurs directeurs des translations : $\vec{u}+\vec{v}$ aura pour coordonnées la somme termes à termes des coordonnées de $\vec{u}$ et de $\vec{v}$.

\begin{de}
    Soient $\vec{u} \Coord{x_\vec{u}}{y_\vec{u}}$ et $\vec{v} \Coord{x_\vec{v}}{y_\vec{v}}$ deux vecteurs du plan. On note $\vec{u}+\vec{v}$ le vecteur de coordonnées $\Coord{x_\vec{u}+x_\vec{v}}{y_\vec{u}+y_\vec{v}}$
\end{de}
...
\begin{prop}[Relation de Chasles]
  Soient $A$, $B$ et $C$ trois points du plan. \vec{AB} + \vec{BC} = \vec{AC}
\end{prop}
...
En plus d'additionner deux vecteurs, on peut également les additionner
